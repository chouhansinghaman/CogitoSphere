        ...
        <div className="group relative w-full h-full flex flex-col">
            ...
            <div className="relative bg-white border border-gray-100 rounded-[2rem] p-6 flex-1 flex flex-col shadow-sm hover:shadow-xl transition-all duration-300">
                ...
                <div className="mb-6 flex-1">
                    ...
                    <p className="text-sm text-gray-500 font-medium leading-relaxed line-clamp-3">{idea.description}</p>
                </div>
                ...
                <div className="mt-auto">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={() => !isFull && onViewDetails(idea)} // If full, clicking only opens details, joining handled inside
...
const IdeaModal = ({ show, onClose, onSave, initialData }) => {
    // ✅ Added maxMembers to state (Default 5)
    ...
    useEffect(() => {
        if(initialData) {
            setFormData({
                ...
                maxMembers: initialData.maxMembers || 5 // Load existing limit
            });
        } else {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
            <div className="bg-white rounded-[2rem] w-full max-w-lg p-8 shadow-2xl relative animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 mt-6">
                    ...
                    {/* ✅ NEW: Split Row for Link & Team Size */}
...
const Community = () => {
    ...
    const handleJoinTeam = async (idea) => {
        // 1. ✅ CHECK: Is Builder Profile Active?
        const isProfileActive = user?.builderProfile?.lookingForTeam; // Assuming this flag exists in your user object
        // If your user object structure is different, check: user.lookingForTeam OR user.skills.length > 0

        if (!isProfileActive) {
            if (window.confirm("⚠️ Profile Inactive!\n\nYou must activate your 'Builder Profile' and add skills before joining a team.\n\nGo to Settings now?")) {
                window.location.href = "/settings"; // Or navigate('/settings')
            }
            return;
        }

        // 2. Open External Link if exists
        ...
        try {
            ...
            if (!res.ok) {
                // 3. ✅ HANDLE 400 ERRORS (Team Full, Already Joined, etc.)
