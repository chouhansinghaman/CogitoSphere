          ...
          <div className="flex flex-col md:flex-row gap-6 border-t border-gray-100 pt-6">
            ...
            <div className="md:w-1/3 space-y-6">
              ...
              <div>
                ...
                <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold border w-full ${profile.builderProfile?.lookingForTeam
            ...
            <div className="md:w-2/3 border-l border-gray-100 md:pl-6 pl-0 border-l-0 md:border-l">
              ...
              {userIdeas.length > 0 ? (
                <div className="space-y-3">
                  {userIdeas.map((idea) => {
                    // Logic: Check if the current profile ID matches the 'postedBy' ID
                    // Handle both populated object ({_id: '...'}) and string ID cases
                    const ownerId = idea.postedBy?._id || idea.postedBy;
                    const isOwner = ownerId.toString() === profile._id.toString();
                    ...
                    return (
                      <div key={idea._id} className="group p-4 rounded-xl border border-gray-100 bg-gray-50 hover:bg-white hover:border-gray-300 hover:shadow-sm transition-all cursor-default">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          {/* Role Badge */}
                          <span className={`shrink-0 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full flex items-center gap-1 ${isOwner
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-400 font-medium">
                          <span className="flex items-center gap-1">
                            <FiUsers size={12} /> {idea.members?.length || 0} Members
                          </span>
                          {/* Only show 'Active' if they are looking for team or it's populated */}
                          <span className="ml-auto text-gray-300 text-[10px] uppercase font-bold group-hover:text-black transition-colors">
                            {isOwner ? "Project Lead" : "Contributor"}
                          </span>
                        </div>
                      </div>
                    );
                ...
                <div className="py-12 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                  <p className="text-gray-400 text-sm font-medium">No active projects found.</p>
                </div>
              )}
            </div>
