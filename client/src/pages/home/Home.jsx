  ...
  return (
    <div className="relative w-full bg-gradient-to-r from-orange-400 to-yellow-300 rounded-xl p-3 flex items-center justify-between text-black shadow-md mb-4 overflow-hidden">
      ...
      <div className="flex flex-col">
        ...
        <span
          key={key}
          className="text-2xl font-bold animate-in fade-in slide-in-from-bottom-2 duration-500"
        >
      ...
      <button
        ...
        className={`px-3 py-1 rounded-md font-semibold transition-all text-sm ${checkedInToday
    ...
    <div className="w-full h-40 bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-2xl mb-6 flex flex-col items-center justify-center p-4 text-white relative shadow-lg hover:shadow-xl transition-all">
      <div className="relative z-10 flex flex-col items-center">
        <div className="text-6xl lg:text-7xl font-mono flex items-baseline leading-none">
          ...
          {ampm && (
            <span className="text-sm font-semibold ml-1 self-end">{ampm}</span>
          )}
        </div>
        ...
        <div className="flex bg-gray-700 rounded-full p-0.5">
          <button
            ...
            className={`text-xs px-2 py-0.5 rounded-full transition-colors ${!is24HourFormat
...
const LearningGoals = () => {
  const [goals, setGoals] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : [{ id: 1, text: "Add your first goal!", completed: false }];
  });
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col gap-2 mb-3">
        {goals.length === 0 ? (
          <p className="text-sm text-gray-400">No goals yet. Add one!</p>
        ) : (
          goals.map((goal) => (
            <div
              key={goal.id}
              className={`p-2 rounded-lg flex items-center justify-between transition-all text-sm ${goal.completed
            ...
            >
              ...
              >
                <input
                  ...
                  className="mr-2 w-4 h-4 text-indigo-500 bg-gray-700 border-gray-600 rounded pointer-events-none"
                />
      ...
      <form onSubmit={handleAddGoal} className="flex gap-2">
        <input
          ...
          className="flex-grow bg-zinc-700 text-white rounded-lg px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <button
          type="submit"
          className="bg-indigo-500 text-white font-semibold px-3 py-1 rounded-lg hover:bg-indigo-600 transition-colors text-sm"
        >
          Add
        </button>
      </form>
...
const ResourceHub = () => {
  ...
  useEffect(() => {
    localStorage.setItem("resourceHub", JSON.stringify(resources));
  }, [resources]);
  ...
  const handleDeleteResource = (idToDelete) => {
    setResources(resources.filter((res) => res.id !== idToDelete));
  };
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="flex flex-col gap-2">
        {resources.length === 0 ? (
          <p className="text-sm text-gray-400">No resources yet. Add one!</p>
        ) : (
          resources.map((res) => (
            <div
              key={res.id}
              className="bg-zinc-800 p-2 rounded-lg flex items-center justify-between gap-2 hover:bg-zinc-700 transition-colors text-sm"
            >
              <a
                href={res.url}
                target="_blank"
          ...
          <div className="bg-zinc-900 text-white rounded-2xl p-6 w-full max-w-md shadow-xl relative">
            ...
            <form onSubmit={handleAddResource} className="flex flex-col gap-3">
              <input
                ...
                className="bg-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <input
                type="url"
              ...
              <div className="flex justify-end gap-2 mt-2">
                <button
                  ...
                  className="px-3 py-1 bg-gray-600 rounded-lg hover:bg-gray-500 transition-colors text-sm"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-3 py-1 bg-indigo-500 rounded-lg hover:bg-indigo-600 transition-colors font-semibold text-sm"
                >
                  Add
                </button>
              </div>
  ...
  return (
    <div className="flex items-center gap-4 w-full mb-4">
      <div className="relative w-16 h-16 group flex-shrink-0">
        <img
          src={user?.avatar || "https://i.pinimg.com/736x/51/19/95/511995729851564ed88c865f42e1844b.jpg"}
          alt={user?.name || "User"}
        ...
        <div
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2H3a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h9"></path><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line><path d="m16 20 2-2-4-4-2 2-4-4"></path></svg>
        </div>
        <input
          type="file"
          ref={fileInputRef}
...
const ProfileCard = ({ user }) => {
  ...
  const [activeTab, setActiveTab] = useState(
    localStorage.getItem("activeTab") || "goals"
  );
  ...
  return (
    <div className="w-full md:w-[380px] bg-black/90 backdrop-blur-xl text-white rounded-3xl p-6 flex flex-col items-center relative shadow-xl">
      <div className="absolute top-4 right-4 flex items-center gap-3">
        <button onClick={() => navigate("/notifications")} className="text-lg">
          🔔
        </button>
      </div>
      ...
      <button
        ...
        className="w-full px-6 py-3 bg-yellow-400 text-black rounded-xl font-semibold hover:bg-yellow-500 transition-colors flex items-center justify-center gap-2 mb-4"
      >
        🏆 Leaderboard
      </button>
      ...
      <div className="w-full bg-zinc-900/80 border border-zinc-800 rounded-2xl p-4 mt-2">
        <div className="flex border-b border-zinc-700 mb-4">
          <button
            ...
            className={`flex-1 py-2 text-sm font-semibold transition-colors ${activeTab === "goals"
          ...
          <button
            ...
            className={`flex-1 py-2 text-sm font-semibold transition-colors ${activeTab === "resources"
              ? "text-white border-b-2 border-indigo-500"
              : "text-gray-400 hover:text-white"
              }`}
          >
        ...
        <div className="w-full">
          {activeTab === "goals" ? <LearningGoals /> : <ResourceHub />}
        </div>
      </div>
    </div>
  );
...
const PerformanceSnapshot = ({ submissions, isLoading }) => {
  const stats = useMemo(() => {
    if (!submissions || submissions.length === 0) {
      return { quizzesTaken: 0, averageScore: 0, highestScore: 0 };
    }
            ...
            <div key={sub._id} className="flex items-center justify-between">
              ...
              <div className="w-1/3 text-right">
                ...
                <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                  <div
                    className={`${sub.percentage >= 80 ? 'bg-green-500' : sub.percentage >= 50 ? 'bg-amber-500' : 'bg-red-500'} h-1.5 rounded-full`}
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      setIsLoading(true);
      try {
        const res = await fetch('http://localhost:5001/api/submissions/my', {
          headers: {
